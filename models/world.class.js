...
class World {
  ...
  /** Checks collisions between the character and all active enemies each frame. */
  checkCollisions() {
    if (this.character.isDead()) return;
    this.level.enemies.forEach((enemy) => {
      if (enemy.isDead() || enemy.isAwake === false) return;
      if (this.character.isCollidingFromAbove(enemy)) {
        this.handleStompEnemy(enemy);
  ...
  /**
   * Handles a successful stomp on an enemy: bounces the character, hits the enemy,
   * plays the bounce sound, spawns a damage text, and checks for boss defeat.
   *
   * @param {MovableObject} enemy - The enemy that was stomped.
   */
  handleStompEnemy(enemy) {
  ...
  /** Applies a hit to the character, updates the health bar, spawns a damage text, and plays the damage sound. */
  handleCharacterHit() {
    this.character.hit();
    ...
    this.damageTexts.push(new DamageText(
      ...
      this.character.y + this.character.offset.top, "-1"
    ));
  ...
  checkThrowObjects() {
    ...
    if (!this.lastThrowTime || now - this.lastThrowTime > 800) {
      const throwX = this.character.otherDirection ? this.character.x - 50 : this.character.x + 100;
  ...
  /** Delegates collectible collision checks to bottle and coin pickup handlers. */
  checkCollectibleCollisions() {
    this.checkBottlePickups();
    this.checkCoinPickups();
  }

  /** Filters collected bottles from the level and updates the bottle status bar. */
  checkBottlePickups() {
  ...
  /** Filters collected coins from the level, updates the coin bar, and awards an extra life at 75%. */
  checkCoinPickups() {
  ...
  checkEndbossTrigger() {
    ...
    if (endboss) endboss.checkTrigger(this.character.x);
  }
  ...
  checkChickenWakeup() {
    if (this.character.x > 400) {
      this.level.enemies.forEach((enemy) => {
        if (enemy instanceof Chicken || enemy instanceof BabyChicken) enemy.wakeUp();
      });
    }
  }
